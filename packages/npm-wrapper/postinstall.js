...
const REPO = 'htekdev/agentic-ops-cli';
...
function getPlatformInfo() {
  ...
  return {
    osName,
    archName,
    binaryName: `agentic-ops-${osName}-${archName}${ext}`,
    ext
  };
}
...
function getLatestRelease() {
  return new Promise((resolve, reject) => {
    const options = {
      ...
      headers: {
        'User-Agent': 'agentic-ops-npm-installer'
      }
    };
...
async function install() {
  console.log('Installing agentic-ops CLI...');
  ...
  try {
    ...
    console.log(`✓ Installed agentic-ops to ${destPath}`);
    ...
    console.error('  1. go install github.com/htekdev/agentic-ops-cli/cmd/agentic-ops@latest');
    console.error('  2. Or download from: https://github.com/htekdev/agentic-ops-cli/releases');
    process.exit(1);
  }
}
