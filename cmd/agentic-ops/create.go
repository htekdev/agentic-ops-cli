...
Examples:
  ...
  agentic-ops create "run eslint on typescript file edits"
  agentic-ops create "validate JSON files before commit"

The generated workflow will be saved to .github/agent-workflows/ and validated
before saving.`,
...
func runCreate(dir, prompt, outputName string, dryRun bool) error {
	...
	// Ensure .github/agent-workflows directory exists
	...
	tempFile := filepath.Join(os.TempDir(), "agentic-ops-validate.yml")
	...
	fmt.Printf("  2. Test it: agentic-ops test --event file --workflow %s\n", outputName)
